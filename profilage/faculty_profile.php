                    ...
                    <div class="card">
                        <div class = "panel panel-primary">
                            <div class = "panel-heading">
                                <h4>PROFILE DE CORPS PROFESSIONNEL PAR UNITE</h4>
                            ...
                            ?>
                                ...
                                <?php
                                    ...
                                    $bsfilter = $con->prepare("SELECT bstotal1.bstotal2 + bstotal3.bstotal4 as bstotal FROM (
                                        ...
                                        FROM   tbl_personnel WHERE date_modified >= '$d1' AND date_modified <= '$d2' AND dept_id = '$dept_id' AND bs_name <> ' ' AND per_designation in ('Permanent' , 'Mi-Temps') 
                                        ) as bstotal1,
                                        (
                                        SELECT COUNT(bs_name) as bstotal4
                                        FROM   tbl_personnel_master_file WHERE date_modified >= '$d1' AND date_modified <= '$d2' AND dept_id = '$dept_id' AND bs_name <> ' ' AND per_designation in ('Permanent' , 'Mi-Temps')
                                        ) as bstotal3 ");
                                        $bsfilter->execute();
                                    ...
                                    $msfilter = $con->prepare("SELECT mstotal1.mstotal2 + mstotal3.mstotal4 as mstotal FROM (
                                        ...
                                        FROM   tbl_personnel WHERE date_modified >= '$d1' AND date_modified <= '$d2' AND dept_id = '$dept_id' AND ms_name <> ' ' AND per_designation in ('Permanent' , 'Mi-Temps') 
                                        ) as mstotal1,
                                        (
                                        SELECT COUNT(ms_name) as mstotal4
                                        FROM   tbl_personnel_master_file WHERE date_modified >= '$d1' AND date_modified <= '$d2' AND dept_id = '$dept_id' AND ms_name <> ' ' AND per_designation in ('Permanent' , 'Mi-Temps')
                                        ) as mstotal3 ");
                                    $msfilter->execute();
                                    ...
                                    $drfilter = $con->prepare("SELECT drtotal1.drtotal2 + drtotal3.drtotal4 as drtotal FROM (
                                        ...
                                        FROM tbl_personnel WHERE date_modified >= '$d1' AND date_modified <= '$d2' AND dept_id = '$dept_id' AND dr_name <> ' ' AND per_designation in ('Permanent' , 'Mi-Temps') 
                                        ) as drtotal1,
                                        (
                                        SELECT COUNT(dr_name) as drtotal4
                                        FROM tbl_personnel_master_file WHERE date_modified >= '$d1' AND date_modified <= '$d2' AND dept_id = '$dept_id' AND dr_name <> ' ' AND per_designation in ('Permanent' , 'Mi-Temps')
                                        ) as drtotal3 ");
                                ...
                                <?php
                                    ...
                                    $bsfilter1 = $con->prepare("SELECT COUNT(bs_name) FROM tbl_personnel WHERE dept_id = '$dept_id' AND bs_name <> ' ' AND per_designation in ('Permanent' , 'Mi-Temps')");
                                    $bsfilter1->execute();
                                    $bsfetch = $bsfilter1->fetchAll(); 
                                        foreach ($bsfetch as $key => $row11) {
                                        $bstotal1 = $row11['COUNT(bs_name)'];

                                    $msfilter1 = $con->prepare("SELECT COUNT(ms_name) FROM tbl_personnel WHERE dept_id = '$dept_id' AND ms_name <> ' ' AND per_designation in ('Permanent' , 'Mi-Temps')");
                                    $msfilter1->execute();
                                    $msfetch = $msfilter1->fetchAll(); 
                                        foreach ($msfetch as $key => $row1) {
                                        $mstotal1 = $row1['COUNT(ms_name)'];

                                    $drfilter1 = $con->prepare("SELECT COUNT(dr_name) FROM tbl_personnel WHERE dept_id = '$dept_id' AND dr_name <> ' '  AND per_designation in ('Permanent' , 'Mi-Temps')");
                                    $drfilter1->execute();
